...
public class Region_bac_onecc {
    ...
    public static void setRange(Double minLowerBound,
        ...
        Region_bac_onecc.minLowerBound = minLowerBound;
        Region_bac_onecc.minUpperBound = minUpperBound;
        Region_bac_onecc.maxLowerBound = maxLowerBound;
        Region_bac_onecc.maxUpperBound = maxUpperBound;
        Region_bac_onecc.avgLowerBound = avgLowerBound;
        Region_bac_onecc.avgUpperBound = avgUpperBound;
        Region_bac_onecc.sumLowerBound = sumLowerBound;
        Region_bac_onecc.sumUpperBound = sumUpperBound;
        Region_bac_onecc.countLowerBound = countLowerBound;
        Region_bac_onecc.countUpperBound = countUpperBound;
    }
    //Set<Integer>
    public Region_bac_onecc(int id){
        numOfAreas = 0;
        average = 0;
        this.id = id;
        this.acceptLow = Region_bac_onecc.avgLowerBound;
        this.acceptHigh = Region_bac_onecc.avgUpperBound;
    ...
    public boolean addArea(Integer id, long minAttrVal, long maxAttrVal, long avgAttrVal, long sumAttrVal, SpatialGrid sg){
        if (areaList.contains(id)){
            ...
            acceptLow = Region_bac_onecc.avgLowerBound * (numOfAreas + 1) - average * numOfAreas;
            acceptHigh = Region_bac_onecc.avgUpperBound * (numOfAreas +1) - average * numOfAreas;
    ...
    public boolean removable(Integer area, ArrayList<Long> minAttr, ArrayList <Long> maxAttr, ArrayList<Long> avgAttr, ArrayList<Long> sumAttr, SpatialGrid sg){
        ...
        if((this.numOfAreas - 1) >= Region_bac_onecc.countLowerBound &&
                ((this.sum - sumAttr.get(area)) >= Region_bac_onecc.sumLowerBound) &&
                ((this.average * numOfAreas - avgAttr.get(area)) / (numOfAreas - 1) >= Region_bac_onecc.avgLowerBound) &&
                ((this.average * numOfAreas - avgAttr.get(area)) / (numOfAreas - 1) <= Region_bac_onecc.avgUpperBound)){
            if(this.min == minAttr.get(area) || this.max == maxAttr.get(area)){
                ...
                if(tmpMin <= Region_bac_onecc.minUpperBound && tmpMin >= Region_bac_onecc.minLowerBound && tmpMax <= Region_bac_onecc.maxUpperBound && tmpMax >= Region_bac_onecc.maxLowerBound){
    ...
    public boolean connectedAfterRemoval(int area, SpatialGrid sg){
        List<Integer> leftAreas = new ArrayList<Integer>();
        for(int i = 0; i < this.areaList.size(); i++){
            leftAreas.add(this.areaList.get(i));
        }
    ...
    public Region_bac_onecc mergeWith(Region_bac_onecc expandR, ArrayList<Long> minAttr, ArrayList<Long> maxAttr, ArrayList<Long> avgAttr, ArrayList<Long> sumAttr, SpatialGrid sg) {
        if(expandR == null){
        ...
        Region_bac_onecc tmpR = new Region_bac_onecc(-1);
